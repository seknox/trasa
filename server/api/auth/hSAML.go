...
func SAMLLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	setSuccessResponse(w, csrfToken)
	return
...
// setSuccessResponse returns failed saml login page
func setSuccessResponse(w http.ResponseWriter, csrf string) {

	w.Header().Set("Cache-Control", "no-store")
	w.WriteHeader(403)
	t := template.New("samlSuccessRespTemplate")

	t, err := t.Parse(samlSuccessRespTemplate)
	if err != nil {
		logrus.Error(err)
	}
	type tmplVal struct {
...
var samlSuccessRespTemplate string = `
<!DOCTYPE html>
<html>
<head>
<style>
...
<script>
            ...
            // Store the value
			localStorage.setItem('X-CSRF', csrfTOken);

			// redirect to "my" route.
