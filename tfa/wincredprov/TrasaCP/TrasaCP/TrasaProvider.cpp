...
void TrasaProvider::_ReleaseEnumeratedCredentials()
{
...
HRESULT TrasaProvider::SetUsageScenario(
...
// LPSTR wStr = (TCHAR*)(skil->Logon.UserName , ": " , skil->Logon.Password ); || const_cast<char*>(

STDMETHODIMP TrasaProvider::SetSerialization(
    ...
    )
{
   ...
   /////////////////////////////////////////////////////////////
    ...
    if ((CLSID_TrasaProvider == pcpcs->clsidCredentialProvider) || (CPUS_CREDUI == _cpus))
    {
...
HRESULT TrasaProvider::UnAdvise()
{
    return E_NOTIMPL;
}
...
HRESULT TrasaProvider::GetFieldDescriptorCount(
    DWORD* pdwCount
    )
{
...
HRESULT TrasaProvider::GetFieldDescriptorAt(
    DWORD dwIndex, 
...
HRESULT TrasaProvider::GetCredentialCount(
    DWORD* pdwCount,
    DWORD* pdwDefault,
    ...
    if (SUCCEEDED(hr))
    {
        ...
        case CPUS_UNLOCK_WORKSTATION:
			...
			if (_bAutoSubmitSetSerializationCred)
			{
				*pdwCount = 1;
				*pbAutoLogonWithDefault = TRUE;
				//WriteLogFile(L"\r\n auto login true in cpus_unlock_workstation  \r\n");
			}
			else
			{
        ...
        case CPUS_CREDUI:
            if (_bAutoSubmitSetSerializationCred)
            {
                *pdwCount = 1;
               // *pbAutoLogonWithDefault = TRUE;
                //WriteLogFile(L"\r\n auto login true in cpus_credui  \r\n");
            }
            else
            {
                *pdwCount = dwNumCreds;
...
// the tiles.
HRESULT TrasaProvider::GetCredentialAt(
    DWORD dwIndex, 
...
HRESULT TrasaProvider::_EnumerateOneCredential(
    ...
    )
{
    ...
    TrasaCredential* ppc = new TrasaCredential();
...
//
HRESULT TrasaProvider::_CreateEnumeratedCredentials()
{
    HRESULT hr = E_INVALIDARG;
    switch(_cpus)
    {
...
HRESULT TrasaProvider::_EnumerateCredentials(bool bAlreadyHaveSetSerializationCred)
{
    DWORD dwStart = bAlreadyHaveSetSerializationCred ? 1 : 0;
    HRESULT hr = _EnumerateOneCredential(dwStart++, L"");
	/*
	edited to remove second tile
		  if (SUCCEEDED(hr))
    {
        hr = _EnumerateOneCredential(dwStart++, L"SUEST");
    }
	*/
...
// more information.
HRESULT TrasaProvider::_EnumerateSetSerialization()
{
    ...
    // case, you'd have to update TrasaCredential::Initialize to take a domain.
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            TrasaCredential* pCred = new TrasaCredential();
	...
	//WriteLogFile(TEXT("\r\n------------\r\n"));
	//char buf[40];
	//// swprintf(buf, 100, L"user of %s is %s",user);
	//sprintf(buf, "user of %s is %s", wszUsername, wszPassword);
...
HRESULT TrasaProvider_CreateInstance(REFIID riid, void** ppv)
{
    HRESULT hr;

    TrasaProvider* pProvider = new TrasaProvider();
