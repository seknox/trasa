// Global session object store
var sessionStore = new Object();
sessionStore['testhost'] = 'testsession';

// initHttpRequest sends login request to trasacore. this login request initiates
// user's session to current web service in context.
async function initHttpRequest(domain, totp) {
  let exToken = await getExtID();
  let data = {
    exToken: exToken,
    domain: domain,
    tfaMethod: 'u2f',
  ...
  let resp = await axios.post(host + '/api/v1/http/init/session', data);
  // console.log('axios resp: ', resp.data)
  if (
    resp.length === 0 ||
    resp.data.status === 'failed' ||
    resp.data.status === 'notrequired' ||
    resp.data.length === 0
  ) {
    // console.log('resp.data is: ', resp.data, ' :returning false')
    return false;
  } else {
    // set session recording map
    srMap.set(resp.data.data[0].sessionID, '');
    srCounter.set(resp.data.data[0].sessionID, 1);

    let buildSession =
      resp.data.data[0].sessionID +
      ':' +
      resp.data.data[0].csrfToken +
      ':' +
      resp.data.data[0].sessionRecord;
    sessionStore[domain] = buildSession;
